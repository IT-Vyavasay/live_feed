...
export async function POST(req, res) {
    try {
        ...
        try {   
            validate_config_number(metaValue, "matrix reward perc")   
            if (parseFloat(metaValue) >100) {
        ...
        let admin = await sql_query("select twoFaCode from tblslr_admin where slrAdminId = ? ", [adm.data.id])  
            ...
            if (twofa) { 
                const metaKey = 'matrixRewardPerc'
                ...
                if (isMetakey) {
                    ...
                    return NextResponse.json({ message: 'Matrix reward config updated successfully'  }, { status: 200 })
                } else {
                    await sql_query("insert into tblslr_config (metaKey,metaValue) values (?,?)", [metaKey, metaValue]) 
                    return NextResponse.json({ message: 'Matrix reward config added successfully'  }, { status: 200 })
                } 
